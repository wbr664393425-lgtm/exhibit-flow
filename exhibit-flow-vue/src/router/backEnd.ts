	...
	return routes
		.map((item: any) => {
			if (item.path && item.path.startsWith('http')) {
				...
				const viewComponent = dynamicImport(dynamicViewsModules, item.path.split('/:')[0]);
				if (viewComponent) item.component = viewComponent;
			}

			if (item.children) {
				item.children = backEndComponent(item.children).filter(Boolean);
				if (item.children.length > 0) {
					item.redirect = item.children[0].path;
					// 目录节点不对应具体页面文件时，回退到通用父级路由容器。
					if (!item.component) item.component = () => import('/@/layout/routerView/parent.vue');
				}
			}

			// 后端菜单仍返回已裁剪业务页面时，自动过滤不可解析的叶子节点。
			if (!item.component && (!item.children || item.children.length === 0)) return null;
			return item;
		})
		.filter(Boolean);
}
