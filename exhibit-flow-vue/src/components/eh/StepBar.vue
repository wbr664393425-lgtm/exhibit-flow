	...
	<div style="display:flex;align-items:center;margin-bottom:24px">
		<template v-for="(s, i) in steps" :key="i">
			<div style="display:flex;align-items:center;gap:8px">
				...
				<span :style="{ fontSize: '12px', fontWeight: i + 1 === cur ? 700 : 400, color: i + 1 === cur ? 'var(--t-accent-strong)' : 'var(--t-text3)', whiteSpace: 'nowrap' }">{{ s }}</span>
			</div>
			<div v-if="i < steps.length - 1" :style="{ flex: 1, height: '1px', background: i + 1 < cur ? 'var(--t-success)' : 'var(--t-border)', margin: '0 10px' }" />
		</template>
	</div>
</template>
